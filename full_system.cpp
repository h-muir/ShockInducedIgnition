//Full-system: HLLC solver for the Euler equations + semi-analytic ODE solver for chemistry
//non-dimensionalised equations
//Use this code for full system simulations with chemistry on -> run "dimensional_fullsystem.cpp"... 
//... for the toro tests with chemistry off
//links to header file: shockTests.H
...
ConsU primtoconsU(Prim W){
    ...
    double E = 0.5*W.rho*pow(W.u,2) + W.p/(gamma*(gamma-1));
...
ConsF primtoconsF(Prim W){
    ...
    double mom = W.rho*pow(W.u,2) + W.p/gamma;
    double E = 0.5*W.rho*pow(W.u,2) + W.p/(gamma*(gamma-1));
    double en = W.u*(E + W.p/gamma);
...
Prim consUtoprim(ConsU U){
    double rho = U.rho;
    double u = U.rhou/rho;
    double p = gamma*(gamma-1)*(U.E - 0.5*rho*pow(u,2));
...
class Cell{
    public:
        ...
        Cell(Prim WL, Prim WR):
            ...
            aL = pow(fabs(WL.p/WL.rho), 0.5); //non-physical states are permitted... 
            aR = pow(fabs(WR.p/WR.rho), 0.5); //...as an intermediate step...
            ...
            Sstar = (WR.p/gamma - WL.p/gamma + WL.rho*WL.u*(SL-WL.u) - WR.rho*WR.u*(SR-WR.u))/\
            ...
            ULstar.E = ULstar.rho*(UL.E/WL.rho+(Sstar-WL.u)*(Sstar + WL.p/(gamma*WL.rho*(SL-WL.u))));
            ...
            URstar.E = URstar.rho*(UR.E/WR.rho+(Sstar-WR.u)*(Sstar + WR.p/(gamma*WR.rho*(SR-WR.u))));
...
vector< vector<Prim> > generate_solution(TestCase CASE, Vector &time_vec, 
    ...
    double c_new, T_new;            //chemistry parameters updated at every sub-cycle step
    Vector C_vec(n,1);
    Vector T_vec(n,1);
    double p_sub;
    double cr, Tr, tr;
...
Vector writetofile(TestCase CASE, vector< vector<Prim> > MATRIX){
    ...
    //initialising these to possible values
    double e;
    for(int i = 2; i < n; ++i){
...
void writetoGNUplot(TestCase CASE, Vector domain, int num_snaps, Vector time_vec, 
    ...
    file << "set terminal gif animate delay 2 \n";
    file << "n = " << nT << '\n';
    ...
    snapfile << "set key below samplen 2 spacing .7 font \",12\" \n";
    snapfile << "set grid ls 0 \n";
    //snapfile << "set xlabel 'x' \n";
    ...
    for(int i = 1; i<num_snaps+1; ++i){
        ...
        snapfile << "'" << p_datfile << "\' using 1:" << index << " title 'T = " << \
           time_vec[index-1] << "\' with lines lc rgb \'red\' ";
        if(i != num_snaps){
            snapfile << ", ";
        }else{
            snapfile << "\n";
    ...
    for(int i = 1; i<num_snaps+1; ++i){
        ...
        snapfile << "'" << u_datfile << "\' using 1:" << index << " title 'T = " << \
           time_vec[index-1] << "\' with lines lc rgb \'gray\' ";
        if(i != num_snaps){
            snapfile << ", ";
        }else{
            snapfile << "\n";
    ...
    for(int i = 1; i<num_snaps+1; ++i){
        ...
        snapfile << "'" << rho_datfile << "\' using 1:" << index << " title 'T = " << \
           time_vec[index-1] << "\' with lines lc rgb \'purple\' ";
        if(i != num_snaps){
            snapfile << ", ";
        }else{
            snapfile << "\n";
    ...
    for(int i = 1; i<num_snaps+1; ++i){
        ...
        snapfile << "'" << c_datfile << "\' using 1:" << index << " title 'T = " << \
           time_vec[index-1] << "\' with lines lc rgb \'blue\' ";
        if(i != num_snaps){
            snapfile << ", ";
        }else{
            snapfile << "\n";
...
void GNUplot_finalT(TestCase CASE, string filetype){
    ...
    file << "set key off \n";
    file << "set xrange[-0.05:1.02] \n";
    file << "set multiplot layout 2,2 \n";
    file << "set title \"Density - rho\" \n";
    //file << "set arrow from 0.475, graph 0 to 0.4755, graph 1 nohead lc rgb \'red\'\n";
    file << "set arrow from 0.0, graph 0 to 0.0, graph 1 nohead lc rgb \'gray\'\n";
    //file << "set arrow from 1.0, graph 0 to 1.0, graph 1 nohead lc rgb \'black\'\n";
...
int main(){
    ...
    TestCase CASE("ShockTest1");            //"ShockTest1" is the studied full-system problem 
    ...
    string BC = "left reflective wall";     //Boundary conditions:
                                            //options:  "left reflective wall" or "transmissive"
    bool wall_correction = 1;               //wall heating correction: on/off 
    bool chemistry = 1;                     //chemistry switch: on/off
    int ns = 5;                             //number of chemistry sub-cycle steps
    ...
    double Vt0 = 1.15;      //`visual t0' first snap shot time
    double Vtf = 1.38;      //'visual tf' final snap shot time
